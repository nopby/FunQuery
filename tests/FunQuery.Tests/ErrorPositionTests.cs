...
public class ErrorPositionTests
{
    ...
    [InlineData("$source([{id:1}]).$filter(not id)", QueryErrorCode.TypeMismatch, 30, 2)]
    [InlineData("$source([{id:1}]).$filter(id in 1)", QueryErrorCode.TypeMismatch, 32, 1)]
    [InlineData("$source([{id:1}]).$filter(id in ['a'])", QueryErrorCode.TypeMismatch, 26, 11)]
    [InlineData("$source([{id:1}]).$filter(id in [[1]])", QueryErrorCode.TypeMismatch, 32, 5)]
    [InlineData("$source([{id:1}]).$filter([1] in [1])", QueryErrorCode.TypeMismatch, 26, 3)]
    [InlineData("$source([{id:1}]).$filter(id contains 'x')", QueryErrorCode.TypeMismatch, 26, 2)]
    [InlineData("$source([{name:'a'}]).$filter(name contains 1)", QueryErrorCode.TypeMismatch, 44, 1)]
    [InlineData("$source([{id:1}]).$filter(id eq not 1)", QueryErrorCode.UnexpectedToken, 32, 3)]
    [InlineData("$source([-a])", QueryErrorCode.UnexpectedCharacter, 9, 1)]
